...
# Trace a key's lifecycle (insert → split → loss)
...
def cmd_correlate(events: Sequence[Event], ikey: int) -> None:
    ...
    if insert_successes:
        print(f"✓ Key was successfully inserted {len(insert_successes)} time(s):")
        for event in insert_successes:
            print(f"  Leaf: {event.leaf_ptr}, Slot: {event.slot}")
            print(f"  Time: {_format_timestamp(event.timestamp)}")
            print(
                f"  Perm: {event.fields.get('old_perm_size')} → {event.fields.get('new_perm_size')}"
            )
    else:
        print("✗ No successful insert found for this key")
        ...
        if later_splits:
            ...
            for event in later_splits:
                ...
                if event.message == MSG_SPLIT_START:
                    ...
                    if found_in_left:
                        print(f"    ✓ Key {ikey} found in LEFT side of split")
                    elif found_in_right:
                        print(f"    ✓ Key {ikey} found in RIGHT side of split")
                    else:
                        print(f"    ⚠️  Key {ikey} NOT FOUND in split distribution!")
...
def cmd_interleave(
    ...
    *,
    ...
    for leaf, threads in contested:
        print(
            f"━━━ Leaf {leaf} ({len(threads)} threads: {', '.join(sorted(threads))}) ━━━"
        )
    ...
    for leaf_ptr, stream in per_leaf.items():
        ...
        for event in stream:
            ...
            if prev != ikey:
                findings += 1
                print(
                    f"⚠️  [slot-reuse] leaf={leaf_ptr} epoch={epoch} slot={slot} "
                    f"prev_ikey={prev} new_ikey={ikey}"
                )
                print(f"    at {_format_timestamp(event.timestamp)}")

    if findings == 0:
        print("✓ No slot-reuse anomalies detected.")
    else:
...
def anomaly_slot_steal(events: Sequence[Event]) -> int:
            ...
            if event.message == MSG_CAS_SUCCESS:
                ...
                if slot in slot_owner:
                    ...
                    if prev_ikey != ikey:
                        ...
                        findings += 1
                        print(f"⚠️  [slot-steal] leaf={leaf_ptr} slot={slot}")
    ...
    if findings == 0:
        print("✓ No slot-stealing anomalies detected.")
    else:
        print(f"\nTotal: {findings} anomalies")
        print(
...
def cmd_blink_warnings(events: Sequence[Event], *, limit: int | None = None) -> None:
    ...
    if not warnings:
        print("✓ No B-link navigation warnings found.")
        print("\nThis means either:")
...
def anomaly_writer_reader_divergence(events: Sequence[Event]) -> int:
        ...
        # B-link warning - check if the leaf differs from insert leaf
            ...
            if ikey in insert_success and reader_leaf:
                ...
                if reader_leaf != insert_leaf:
                    ...
                    findings += 1
                    print(f"⚠️  [writer-reader-divergence] ikey={ikey}")
                    ...
                    if next_ptr == insert_leaf:
                        print(
                            f"    ⚡ Insert leaf is in B-link chain (next_ptr={next_ptr})"
                        )
                        print(
    ...
    if findings == 0:
        print("✓ No writer/reader leaf divergence detected.")
...
def anomaly_immediate_verify(events: Sequence[Event]) -> int:
    ...
    if not immediate_failures:
        print("✓ No immediate verification failures detected.")
    ...
    for ikey, time_diff, warning in immediate_failures[:20]:
        ...
        if insert_leaf != reader_leaf:
            print("    ⚠️  LEAF MISMATCH - key routed to wrong leaf!")
...
def anomaly_missing_after_success(events: Sequence[Event]) -> int:
    ...
    for split_event in splits:
        ...
        if missing:
            ...
            print(f"⚠️  [missing-after-success] leaf={left_ptr}")
    ...
    if findings == 0:
        print("✓ No missing-after-success anomalies detected.")
    else:
