...
COLD_PATTERNS = [
    ...
    r"^create_",
    r"^build_",
    r"^make_",  # Construction
    r"^default$",
    r"^from_",
    r"^into_",
    r"^try_from",
    r"^try_into",  # Conversion
    r"_new$",
    r"_create$",
    r"_init$",  # Construction suffixes
    r"^drop$",
    r"^clone$",  # Lifecycle
    r"^debug$",
    r"^display$",
    ...
    r"^fail",
    r"^panic",
    r"^abort",  # Error handling
    r"split",
    r"merge",
    r"rebalance",
    r"compact",  # Tree operations (rare)
    r"^serialize",
    r"^deserialize",  # Serialization
    r"^alloc",
    r"^dealloc",
    r"^free",  # Allocation
    r"^retire",
    r"^reclaim",  # Memory reclamation
    r"^init",
    r"^setup",
    r"^teardown",  # Initialization
]
...
HOT_PATTERNS = [
    ...
    r"^size$",
    r"^count$",
    r"^capacity$",  # Size queries
    r"^empty$",
    r"^is_empty$",
    r"^is_full$",  # State queries
    r"^ptr$",
    r"^as_ptr",
    r"^as_ref",
    r"^as_mut",  # Pointer access
    r"^version",
    r"^perm",
    r"^slot",
    r"^ikey",
    r"^child",  # Domain-specific hot paths
    r"^next$",
    r"^prev$",
    r"^first$",
    r"^last$",  # Iteration
    r"^push$",
    r"^pop$",
    r"^peek$",  # Stack/queue ops
    r"^insert$",
    r"^remove$",
    r"^update$",  # Modification (can be hot)
    r"^index$",
    r"^at$",  # Indexing
    r"^cast$",
    r"^transmute",  # Type conversion
...
BLOAT_WARNING_CALL_SITES = (
    10  # Only consider bloat for functions with >= this many calls
)
...
def extract_function_info(
    content: str, attr_line_idx: int, inline_type: InlineType
) -> Optional[tuple]:
...
def count_call_sites(
    content: str, fn_name: str, exclude_definition: bool = True
) -> int:
...
def find_issues(func: FunctionInfo) -> list[Issue]:
    ...
    if func.inline_type == InlineType.INLINE_ALWAYS:
        ...
        # Small hot-path functions (getters, etc.) are fine even with many call sites
            ...
            func.lines >= BLOAT_WARNING_LINES
            and func.call_sites >= BLOAT_WARNING_CALL_SITES
            and func.code_bloat > BLOAT_WARNING_THRESHOLD
            and func.path_type != PathType.HOT
        ):
            issues.append(
                Issue(
                    func=func,
        ...
        if (
            func.lines <= SMALL_FUNCTION_LINES
            and func.path_type == PathType.HOT
            and func.is_pub
        ):
            issues.append(
...
def analyze_directory(path: Path) -> AnalysisResult:
    ...
    # Calculate stats
    inline_always_count = sum(
        1 for f in result.functions if f.inline_type == InlineType.INLINE_ALWAYS
    )
    inline_count = sum(
        1 for f in result.functions if f.inline_type == InlineType.INLINE
    )
    inline_never_count = sum(
        1 for f in result.functions if f.inline_type == InlineType.INLINE_NEVER
    )
...
def format_output(result: AnalysisResult, args: argparse.Namespace) -> str:
    ...
    output.append(
        f"Estimated code bloat:  {result.stats['estimated_total_bloat']} lines"
    )
    output.append("")
    ...
    for severity in [Severity.ERROR, Severity.WARNING, Severity.INFO]:
        ...
        for issue in sorted(severity_issues, key=lambda x: -x.func.lines):
            output.append(f"\n{issue.func.location}")
            output.append(
                f"  fn {issue.func.name}() - {issue.func.lines} lines, {issue.func.call_sites} call sites"
            )
    ...
    if args.top_bloat > 0:
        ...
        output.append(
            f"{'Location':<45} {'Function':<25} {'Lines':>6} {'Calls':>6} {'Bloat':>8}"
        )
        output.append("-" * 95)
    ...
    if args.cold_path:
        ...
        cold_funcs = [
            f
            for f in result.functions
            if f.inline_type == InlineType.INLINE_ALWAYS
            and f.path_type == PathType.COLD
        ]
    ...
    if args.verbose and not args.issues_only:
        ...
        _headers = ["Location", "Function", "Type", "Lines", "Calls", "Bloat", "Path"]
        rows = []
