...
use masstree::{
    InlineSuffixBag, MassTree15, MassTree24, Permuter15, Permuter24, internode::InternodeNode,
    nodeversion::NodeVersion, value::LeafValue,
};
use std::mem::size_of;
...
fn main() {
    ...
    // InternodeNode
    println!(
        "InternodeNode<LeafValue<u64>>: {} bytes ({} cache lines)",
        size_of::<InternodeNode<LeafValue<u64>>>(),
        size_of::<InternodeNode<LeafValue<u64>>>() / 64
    );
    println!(
        "InternodeNode alignment: {} bytes",
        std::mem::align_of::<InternodeNode<LeafValue<u64>>>()
    );
    println!();
    ...
    // InlineSuffixBag
    println!(
        "InlineSuffixBag<15, 256>: {} bytes",
        size_of::<InlineSuffixBag<15, 256>>()
    );
    println!();
    ...
    offset += 4;
    println!(
        "  [{:3}..{:3}] modstate: AtomicU8 (1 byte)",
        offset,
        offset + 1
    );
    offset += 1;
    println!(
        "  [{:3}..{:3}] _pad0: [u8; 55] (55 bytes)",
        offset,
        offset + 55
    );
    offset += 55;
    ...
    // Alignment check
    let cl1_start = offset.div_ceil(cache_line) * cache_line;
    if cl1_start > offset {
        println!(
            "  [{:3}..{:3}] implicit padding ({} bytes)",
            offset,
            cl1_start,
            cl1_start - offset
        );
    ...
    println!(
        "  [{:3}..{:3}] permutation: AtomicU64 (8 bytes)",
        offset,
        offset + 8
    );
    offset += 8;
    println!(
        "  [{:3}..{:3}] _pad1: [u8; 56] (56 bytes)",
        offset,
        offset + 56
    );
    offset += 56;
    ...
    println!(
        "  [{:3}..{:3}] ikey0: [AtomicU64; 15] (120 bytes)",
        offset,
        offset + 120
    );
    offset += 120;
    ...
    println!(
        "  [{:3}..{:3}] keylenx: [AtomicU8; 15] (15 bytes)",
        offset,
        offset + 15
    );
    offset += 15;
    ...
    // leaf_values
    println!(
        "Cache Lines 4-6 (bytes {}..{}): Values",
        offset,
        offset + 120
    );
    println!(
        "  [{:3}..{:3}] leaf_values: [AtomicPtr<u8>; 15] (120 bytes)",
        offset,
        offset + 120
    );
    offset += 120;
    ...
    println!(
        "  [{:3}..{:3}] inline_ksuf: InlineSuffixBag<15, 256>",
        offset,
        offset + inline_total
    );
    println!("             - slots: [InlineSlotMeta; 15] ({inline_slots} bytes)");
    println!("             - size: u16 ({inline_size} bytes)");
    println!("             - data: [u8; 256] ({inline_data} bytes)");
    println!("             - total: {inline_total} bytes");
    offset += inline_total;
    println!();
    ...
    println!(
        "  [{:3}..{:3}] external_ksuf: AtomicPtr (8 bytes)",
        offset,
        offset + 8
    );
    offset += 8;
    println!(
        "  [{:3}..{:3}] next: AtomicPtr (8 bytes)",
        offset,
        offset + 8
    );
    offset += 8;
    println!(
        "  [{:3}..{:3}] prev: AtomicPtr (8 bytes)",
        offset,
        offset + 8
    );
    offset += 8;
    println!(
        "  [{:3}..{:3}] parent: AtomicPtr (8 bytes)",
        offset,
        offset + 8
    );
    offset += 8;
    ...
    let final_aligned = offset.div_ceil(cache_line) * cache_line;
    if final_aligned > offset {
        println!(
            "  [{:3}..{:3}] tail padding ({} bytes)",
            offset,
            final_aligned,
            final_aligned - offset
        );
        offset = final_aligned;
    }

    #[expect(clippy::cast_precision_loss)]
    let cache_lines_used = offset as f64 / 64.0;

    println!("=== Summary ===\n");
    println!("Calculated LeafNode15 size: {offset} bytes");
    println!("Cache lines used: {cache_lines_used:.1}");
    println!();
    ...
    println!();
    println!(
        "LeafNode15:    ~{} bytes ({} CL) - trie-of-B+trees design",
        offset,
        offset / 64
    );
    println!();
