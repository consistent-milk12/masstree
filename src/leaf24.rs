...
impl<S: ValueSlot> LeafNode24<S> {
    ...
    pub unsafe fn assign_ksuf_init(&self, slot: usize, suffix: &[u8], guard: &LocalGuard<'_>) {
        debug_assert!(
            slot < WIDTH_24,
            "assign_ksuf_init: slot {slot} >= WIDTH_24 {WIDTH_24}"
        );
        debug_assert!(
            self.version.is_locked() || self.version.is_unpublished(),
    ...
    pub unsafe fn try_assign_ksuf(
        ...
        debug_assert!(
            slot < WIDTH_24,
            "try_assign_ksuf: slot {slot} >= WIDTH_24 {WIDTH_24}"
        );
        debug_assert!(
            self.version.is_locked() || self.version.is_unpublished(),
    ...
    unsafe fn try_assign_ksuf_slow(
        ...
        let new_ptr: *mut SuffixBag<WIDTH_24> =
    ...
    pub unsafe fn try_assign_ksuf_init(
        ...
        debug_assert!(
            slot < WIDTH_24,
            "try_assign_ksuf_init: slot {slot} >= WIDTH_24 {WIDTH_24}"
        );
        debug_assert!(
            self.version.is_locked() || self.version.is_unpublished(),
    ...
    unsafe fn try_assign_ksuf_init_slow(
        ...
        let mut new_bag: SuffixBag<WIDTH_24> = inline.drain_to_external_init(slot, suffix)?;
    ...
    unsafe fn split_into_preallocated(
        ...
        unsafe {
            ...
            for i in 0..entries_to_move {
                ...
                // Verify immediately after store
                #[cfg(feature = "debug-routing")]
                {
                ...
                if keylenx == KSUF_KEYLENX {
                    if let Some(suffix) = self.ksuf(old_slot) {
                        // Pass initializing=true: slots 0..new_slot are filled sequentially
