//! Concurrent benchmarks for MassTree24 (WIDTH=24) - Criterion version
//!
//! Equivalent to concurrent_masstree24.rs but using Criterion for more stable measurements.
//!
//! ## Running
//!
//! ```bash
//! cargo bench --bench concurrent_masstree24_crit
//! cargo bench --bench concurrent_masstree24_crit --features mimalloc
//! cargo bench --bench concurrent_masstree24_crit -- --save-baseline main
//! cargo bench --bench concurrent_masstree24_crit -- --baseline main
//! ```
//!
...
fn bench_01_concurrent_writes_disjoint(c: &mut Criterion) {
    ...
    for threads in [1, 2, 3, 4, 5, 6] {
        group.bench_function(BenchmarkId::new("masstree24", threads), |b| {
            b.iter_with_setup(
...
fn bench_02_concurrent_writes_contention(c: &mut Criterion) {
    ...
    for threads in [1, 2, 3, 4, 5, 6] {
        ...
        group.bench_function(BenchmarkId::new("masstree24", threads), |b| {
            let keys = Arc::clone(&keys);
            b.iter_with_setup(
                || Arc::new(setup_masstree24::<8>(&keys)),
                |tree| {
...
fn bench_03_single_threaded_insert(c: &mut Criterion) {
    ...
    c.bench_function("03_single_threaded_insert/masstree24", |b| {
        b.iter(|| {
...
fn bench_05_get_by_key_size(c: &mut Criterion) {
    ...
    {
        let keys = keys::<8>(N);
        let tree = setup_masstree24::<8>(&keys);
        let lookup_keys = uniform_indices(N, 1000, 42);

        group.bench_function("masstree24_8B", |b| {
            b.iter(|| {
                let guard = tree.guard();
                let mut sum = 0u64;
    ...
    {
        let keys = keys::<16>(N);
        let tree = setup_masstree24::<16>(&keys);
        let lookup_keys = uniform_indices(N, 1000, 42);

        group.bench_function("masstree24_16B", |b| {
            b.iter(|| {
                let guard = tree.guard();
                let mut sum = 0u64;
                for &idx in &lookup_keys {
    ...
    {
        let keys = keys::<24>(N);
        let tree = setup_masstree24::<24>(&keys);
        let lookup_keys = uniform_indices(N, 1000, 42);

        group.bench_function("masstree24_24B", |b| {
            b.iter(|| {
                let guard = tree.guard();
                let mut sum = 0u64;
                for &idx in &lookup_keys {
    ...
    {
        let keys = keys::<32>(N);
        let tree = setup_masstree24::<32>(&keys);
        let lookup_keys = uniform_indices(N, 1000, 42);

        group.bench_function("masstree24_32B", |b| {
            b.iter(|| {
                let guard = tree.guard();
                let mut sum = 0u64;
                for &idx in &lookup_keys {
...
fn bench_06_insert_by_key_size(c: &mut Criterion) {
    ...
    for (name, key_size) in [("8B", 8), ("16B", 16), ("24B", 24), ("32B", 32)] {
        match key_size {
            8 => {
                let keys = keys::<8>(N);
                group.bench_function(format!("masstree24_{name}"), |b| {
                    b.iter_with_setup(
                        || keys.clone(),
                        |keys| {
                            let tree = MassTree24::<u64>::new();
...
fn bench_07_concurrent_reads_scaling(c: &mut Criterion) {
    ...
    let tree = Arc::new(setup_masstree24::<8>(&keys));
...
fn bench_08_concurrent_reads_long_keys(c: &mut Criterion) {
    ...
    let tree = Arc::new(setup_masstree24::<32>(&keys));
    let indices = Arc::new(uniform_indices(N, OPS_PER_THREAD, 42));
    ...
    for threads in [1, 2, 3, 4, 5, 6] {
        ...
        group.bench_function(BenchmarkId::new("masstree24_32b", threads), |b| {
            let keys = Arc::clone(&keys);
            let tree = Arc::clone(&tree);
            let indices = Arc::clone(&indices);
...
fn bench_12a_string_values_read(c: &mut Criterion) {
    ...
    let tree = Arc::new(setup_masstree24_string::<16>(&keys));
    ...
    for threads in [1, 2, 3, 4, 5, 6] {
        ...
        group.throughput(Throughput::Elements((threads * OPS_PER_THREAD) as u64));
        group.bench_function(BenchmarkId::new("masstree24_string", threads), |b| {
            let keys = Arc::clone(&keys);
            let tree = Arc::clone(&tree);
            let indices = Arc::clone(&indices);
