...
//!
//! Same benchmarks as `concurrent_read_write.rs` but with:
...
//! 4. **Reduced samples**: 20 samples for quick iteration
//! 5. **Min time**: 1 second minimum per benchmark for speed
//!
//! ## Running
//!
//! ```bash
//! cargo bench --bench concurrent_read_write_light
//! cargo bench --bench concurrent_read_write_light -- 01_
//! ```
...
mod mixed_high_contention {
    ...
    const N: usize = 250; // Small key space = high contention
...
mod insert_only_fair {
    ...
    const N: usize = 25_000; // Pre-populated keys for reads
    const OPS_PER_THREAD: usize = 6_250;
