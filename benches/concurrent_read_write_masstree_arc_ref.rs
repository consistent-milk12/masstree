                        ...
                        thread::spawn(move || {
                                ...
                                warmup_is_write.iter().enumerate().take(WARMUP_OPS)
                            {
                                if is_write {
                                    ...
                                    black_box(tree.get_ref(&hot_key, &guard));
                                }
                            }
                            warmup_done.wait();
...
mod keys_8byte {
    ...
    fn setup_masstree15_8(keys: &[[u8; KEY_SIZE_8]]) -> MassTree15<u64> {
        let tree = MassTree15::new();
        {
            let guard = tree.guard();
            for (i, key) in keys.iter().enumerate() {
                        ...
                        thread::spawn(move || {
                            ...
                            for i in 0..WARMUP_OPS {
                                ...
                                if warmup_is_write[i] {
                                    ...
                                    black_box(tree.get_ref(&read_keys[idx], &guard));
                                } else {
                                    black_box(tree.get_ref(&read_keys[idx], &guard));
                                }
                            }
                            ...
                            for i in 0..OPS_PER_THREAD {
                                if is_write[i] && write_idx < writes_per_thread {
                                    ...
                                    if let Some(v) = tree.get_ref(&read_keys[idx], &guard) {
                                        sum = sum.wrapping_add(*v);
                                    }
                                }
                            }
...
mod pure_insert {
    ...
    fn masstree15(bencher: Bencher, threads: usize) {
        ...
        bencher
            .counter(divan::counter::ItemsCount::new(threads * OPS_PER_THREAD))
            .with_inputs(MassTree15::<u64>::new)
            .bench_local_values(|tree| {
